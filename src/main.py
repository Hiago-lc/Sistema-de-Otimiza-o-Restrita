...
# --- Integrante 4 (Critério de Parada) ---
...
def metodo_barreira(funcao, restricoes, variaveis, ponto_inicial,
    ...
    for k in range(max_iter):
        ...
        # Integrando o critério do Integrante 4 avaliando f(x)
...
def metodo_gradiente(funcao, funcao_expressao, var_x, var_y, ponto_inicial, passo=1, tipo_otimizacao="minimo"):
    ...
    # Inicializando variável para critério de parada (Integrante 4)
    f_anterior = None 

    while iteracao <= max_iteracoes:
        # 1. Calcula o gradiente 
        # (Correção no desempacotamento de x_atual para lambdify rodar sem erro)
        try:
        ...
        # produto interno do gradiente com a direção (Corrigida a variável 'gradiente' para 'vetor_gradiente')
        produto_interno = np.dot(vetor_gradiente, direcao)

        while passo_k > 1e-8:
            f_atual = funcao(x_atual)
            # Corrigida a variável 'x' para 'x_atual'
        ...
        # Aplicação do critério de parada (Integrante 4)
        if criterio_parada_funcao_objetivo(f_novo, f_atual, tolerancia):
